...
{
	::ZeroMemory(&m_PostProcessingInfo, sizeof(CB_POSTPROCESSING_INFO));
}
...
void CPostProcessingShader::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[0].BaseShaderRegister = 5; //b5 : PostProcessingInfo
	pd3dDescriptorRanges[0].RegisterSpace = 0;
	pd3dDescriptorRanges[0].OffsetInDescriptorsFromTableStart = 0;

	pd3dDescriptorRanges[1].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[1].NumDescriptors = 2;
	pd3dDescriptorRanges[1].BaseShaderRegister = 16; //Texture
	...
	pd3dRootParameters[0].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[0]; //PostProcessingInfo + Texture
...
{
	return(CShader::CompileShaderFromFile(L"hlsl/Shaders.hlsl", "PSTextureToFullScreenWithPostProcessing", "ps_5_1", ppd3dShaderBlob));
}
...
{
	UINT ncbElementBytes = ((sizeof(CB_POSTPROCESSING_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbPostProcessingInfo = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

	m_pd3dcbPostProcessingInfo->Map(0, NULL, (void **)&m_pcbMappedPostProcessingInfo);
}

void CPostProcessingShader::UpdateShaderVariables(ID3D12GraphicsCommandList * pd3dCommandList)
{
	memcpy(m_pcbMappedPostProcessingInfo, &m_PostProcessingInfo, sizeof(CB_POSTPROCESSING_INFO));
}

void CPostProcessingShader::ReleaseShaderVariables()
{
	if (m_pd3dcbPostProcessingInfo)
	{
		m_pd3dcbPostProcessingInfo->Unmap(0, NULL);
		m_pd3dcbPostProcessingInfo->Release();
	}
}
...
void CPostProcessingShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	m_PostProcessingInfo.m_bActive = false;

	UINT ncbElementBytes = ((sizeof(CB_POSTPROCESSING_INFO) + 255) & ~255); //256의 배수

	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	CreateCbvAndSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, 1, m_pTexture->GetTextureCount());
	CreateConstantBufferViews(pd3dDevice, pd3dCommandList, 1, m_pd3dcbPostProcessingInfo, ncbElementBytes);
	CreateShaderResourceViews(pd3dDevice, pd3dCommandList, m_pTexture, 1, false);
}
...
void CPostProcessingShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	UpdateShaderVariables(pd3dCommandList);
	pd3dCommandList->SetGraphicsRootDescriptorTable(0, GetGPUCbvDescriptorStartHandle());
