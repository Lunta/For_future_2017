...
void CGameObject::SetLook(XMFLOAT3& xmf3Look)
{
	...
	look = Vector3::CrossProduct(right, GetUp(), true);
...
void CGunshipHellicopter::Animate(float fTimeElapsed)
{
	...
	if (m_pHellfileMissileFrame)
		if (m_bLaunch)
		{
			XMFLOAT3 xmf3Look = m_pHellfileMissileFrame->GetLook();
			xmf3Look = Vector3::ScalarProduct(xmf3Look, 50.f * fTimeElapsed);
			m_pHellfileMissileFrame->m_xmf4x4ToParentTransform._41 += xmf3Look.x;
			m_pHellfileMissileFrame->m_xmf4x4ToParentTransform._42 += xmf3Look.y;
			m_pHellfileMissileFrame->m_xmf4x4ToParentTransform._43 += xmf3Look.z;
		}
		else MoveForward(20.f * fTimeElapsed);
...
void CGunshipHellicopter::UpdateTransform(XMFLOAT4X4 * pxmf4x4Parent)
{
	if (m_bLaunch)
	{
		m_xmf4x4World = m_xmf4x4ToParentTransform;

		if (m_pSibling) m_pSibling->UpdateTransform(pxmf4x4Parent);
		if (m_pChild) m_pChild->UpdateTransform(&m_xmf4x4World);
		m_pHellfileMissileFrame->m_xmf4x4World = m_pHellfileMissileFrame->m_xmf4x4ToParentTransform;
		m_pHellfileMissileFrame->SetLook(GetLook());
	}
	else
	{
		m_xmf4x4World = (pxmf4x4Parent) ? Matrix4x4::Multiply(m_xmf4x4ToParentTransform, *pxmf4x4Parent) : m_xmf4x4ToParentTransform;
...
XMFLOAT3 CGunshipHellicopter::GetMissilePos()
{
	return XMFLOAT3(m_pHellfileMissileFrame->m_xmf4x4ToParentTransform._41, m_pHellfileMissileFrame->m_xmf4x4ToParentTransform._42, m_pHellfileMissileFrame->m_xmf4x4ToParentTransform._43);
}
	...
	: CGameObject(0)
{
	...
	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/Terrain/BaseTexture.dds", 0);
	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/Terrain/DetailTexture.dds", 1);
	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/Terrain/DetailNormalTexture.dds", 2);
	...
	pShader->CreateShaderResourceViews(pd3dDevice, pd3dCommandList, pTerrainTexture, 5, false);

	SetCbvGPUDescriptorHandle(pShader->GetGPUCbvDescriptorStartHandle());

	CMaterial *pTerrainMaterial = new CMaterial();
	pTerrainMaterial->SetTexture(pTerrainTexture);
	pTerrainMaterial->SetReflection(0);
...
CSkyBox::CSkyBox(
	...
	: CGameObject(6)
{
	CRectMeshTextured *pSkyBoxMesh = new CRectMeshTextured(pd3dDevice, pd3dCommandList, 20.0f, 20.0f, 0.0f, 0.0f, 0.0f, +10.0f);
	SetMesh(0, pSkyBoxMesh);
	pSkyBoxMesh = new CRectMeshTextured(pd3dDevice, pd3dCommandList, 20.0f, 20.0f, 0.0f, 0.0f, 0.0f, -10.0f);
	SetMesh(1, pSkyBoxMesh);
	pSkyBoxMesh = new CRectMeshTextured(pd3dDevice, pd3dCommandList, 0.0f, 20.0f, 20.0f, -10.0f, 0.0f, 0.0f);
	SetMesh(2, pSkyBoxMesh);
	pSkyBoxMesh = new CRectMeshTextured(pd3dDevice, pd3dCommandList, 0.0f, 20.0f, 20.0f, +10.0f, 0.0f, 0.0f);
	SetMesh(3, pSkyBoxMesh);
	pSkyBoxMesh = new CRectMeshTextured(pd3dDevice, pd3dCommandList, 20.0f, 0.0f, 20.0f, 0.0f, +10.0f, 0.0f);
	...
	CTexture *pSkyBoxTexture = new CTexture(6, RESOURCE_TEXTURE2D_ARRAY, 0);
	...
	pSkyBoxMaterial->SetReflection(0);
