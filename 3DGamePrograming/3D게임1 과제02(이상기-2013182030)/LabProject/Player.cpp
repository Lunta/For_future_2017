...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	if (dwDirection)
	{
		...
		if (dwDirection & DIR_FORWARD) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Look, 1.0f);
		if (dwDirection & DIR_BACKWARD) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Look, -1.0f);
		//화살표 키 ‘→’를 누르면 로컬 x-축 방향으로 이동한다. ‘←’를 누르면 반대 방향으로 이동한다.
		if (dwDirection & DIR_RIGHT) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Right, 1.0f);
		if (dwDirection & DIR_LEFT) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Right, -1.0f);
		//‘Page Up’을 누르면 로컬 y-축 방향으로 이동한다. ‘Page Down’을 누르면 반대 방향으로 이동한다.
		if (dwDirection & DIR_UP) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Up, 1.0f);
		...
		xmf3Shift = Vector3::ScalarProduct(xmf3Shift, fDistance, true);
	...
	*pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	...
	SetOOBB(pAirplaneMesh->GetBoundingBox());
